...
export default class BasicEdition extends React.Component {
    ...
    equalState(props) {
    	...
    	/*
    	let newName = this.state.name !== props.dataBasic.name;
    	let newQuantity = this.state.quantity.new && props.dataBasic.quantity.new && (this.state.quantity.new !== props.dataBasic.quantity.new || this.state.quantity.old !== props.dataBasic.quantity.old);
    	let newPrice = this.state.price.new && props.dataBasic.price.new && (this.state.price.new !== props.dataBasic.price.new || this.state.price.old !== props.dataBasic.price.old);
    	let finalDiff = (newId || newName || newQuantity || newPrice);
    	if (props.dataReceived && finalDiff && !this.state.doNotUpdateProps) {
    	*/
	...
	save() {
		...
		this.setState({
			...
			.then((response) => {
				var messageType = Boolean(response.data.success) ? 'success' : 'error';
	...
	setMessage(type, text) {
		...
		let nextId = type === 'success' ? null : this.state.id;
		this.setState({
			message: text,
    					...
    					<Modal.Footer>
    						<div class="col-xs-12 pull-left marginTop10px">
    							<div class="col-xs-12 col-md-3 pull-left">
    								<input class="form-control btn btn-primary pull-right" disabled={ disabledSave } type="button" value="Zapisz" onClick={ this.save.bind(this) } />
    							</div>
